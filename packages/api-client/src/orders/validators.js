...
const isDeliveryMode = (value) => value === 'instant' || value === 'scheduled';
const isPublicOrder = (value) => {
    if (!isRecord(value)) {
        return false;
    }
...
const isPricingConfig = (value) => {
    if (!isRecord(value)) {
        return false;
    }
    return typeof value.deliveryFee === 'number' && typeof value.discountRate === 'number';
};
export const parsePublicPricingConfigResponse = (value) => {
...
export const parseCreatePublicOrderResponse = (value) => {
