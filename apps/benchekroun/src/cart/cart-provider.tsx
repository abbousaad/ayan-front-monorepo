...
export function CartProvider({ children }: CartProviderProps) {
  ...
  const value = useMemo<CartContextValue>(() => {
    ...
    return {
      addCartItem: (item) => {
        dispatch({ type: 'add-item', payload: { item, ...createMeta() } });
      },
      cartCount: getCartCount(state),
      clearCartItems: () => {
        dispatch({ type: 'clear-cart', payload: createMeta() });
      },
      closeCart: () => {
        setIsOpen(false);
      },
      decrementCartItem: (productId) => {
        dispatch({ type: 'decrement-item', payload: { productId, ...createMeta() } });
      },
      getItemQuantity: (productId) => getCartItemQuantity(state, productId),
      incrementCartItem: (productId) => {
        dispatch({ type: 'increment-item', payload: { productId, ...createMeta() } });
      },
      ...
      removeCartItem: (productId) => {
        dispatch({ type: 'remove-item', payload: { productId, ...createMeta() } });
      },
      setCartItemQuantity: (productId, quantity) => {
        dispatch({ type: 'set-quantity', payload: { productId, quantity, ...createMeta() } });
      },
      state,
