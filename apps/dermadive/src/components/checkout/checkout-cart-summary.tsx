...
export function CheckoutCartSummary() {
  ...
  return (
    <div className="space-y-4">
      <h2 className="font-display text-xl font-semibold uppercase text-encre">{copy.yourOrder}</h2>
      ...
      <div className="space-y-3">
        {state.items.map((item) => {
          ...
          return (
            <article key={item.productId} className="flex gap-3 border border-brume bg-blanc p-3">
              <img
                alt={item.name}
              ...
              <div className="flex min-w-0 flex-1 flex-col gap-3">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <p className="text-sm font-semibold text-encre">{price(lineTotal, item.currencyCode)}</p>
                </div>
              </div>
            </article>
          );
